 ...
 ****************************************************************************/
int
stack_arp_resolve(PyObject *adapter,
    ...
    ipv4address_t your_ipv4, macaddress_t *your_mac_address)
{
     ...
     */
    int link_type =0;
    PyArg_Parse(PyObject_GetAttrString(adapter, "link_type"), "I", &link_type);
    if (link_type == 12) {
     ...
     */
    PyObject *pcap_py = PyObject_GetAttrString(adapter, "pcap");
    pcap_t *pcap = PyCapsule_GetPointer(pcap_py, "pcap_t");
    rawsock_send_packet(pcap, arp_packet, 60);
    start = time(0);
    i = 0;
    for (;;) {
        unsigned length;
